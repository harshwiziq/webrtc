define( function(require){	
	...
	function set_meta( vc_id, _key, value, is_req) {	
		...
		switch( _key){
			...
			case 'speaker'	 	:
			case 'microphone'	:
				if( is_req){		  /* _key shouldn't be audio-level as it is view only */
			...
			case 'write-control':
				if( is_req){
					/* just shout: 'someone give this user writing controls' and the duty is done. */
					people_ev.emit('perms',{ vc_id : vc_id, key : _key, value : value });	
				}
				else{
					people_ev.emit('control_changed', { vc_id : vc_id, known_key : _key, value : value});
					store[vc_id].meta[ _key] = value;
			...
			case 'kick':
				if (is_req) {
					people_ev.emit('kick', { vc_id : vc_id, key : _key, value : value });
				}
				else {
					people_ev.emit('control_changed', { vc_id : vc_id, known_key : _key, value : value});
					store[vc_id].meta[ _key] = value;
