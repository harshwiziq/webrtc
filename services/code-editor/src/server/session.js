...
//(function() {
  ...
  exports.handler = function(session, createAgent) {
    ...
    handleMessage = function(query) {
      ...
      error = null;
	  console.log("info received handleMessage : " + JSON.stringify(query));
      if (!(query.doc === null || typeof query.doc === 'string' || (query.doc === void 0 && lastReceivedDoc))) {
        error = 'Invalid docName';
		console.log("error handleMessage : " + error);
      }
    ...
    send = function(response) {
      ...
      if (session.ready()) {
		  console.log("send info : " + JSON.stringify(response));
	...
	console.log("failAuthentication :: " + error);
      session.send({
        auth: null,
        error: error
      });
    ...
    session.on('message', bufferMsg = function(msg) {
	  console.log("info received session : " + JSON.stringify(msg));
      if (typeof msg.auth !== 'undefined') {
        ...
        return createAgent(data, function(error, agent_) {
          ...
          if (error) {
            ...
            agent = agent_;
			console.log("sending auth : " + agent.sessionId);
            session.send({
