// Generated by CoffeeScript 1.4.0
  var AUTH_TIMEOUT, hat, syncQueue, log;
  ...
  exports.handler = function(session, createAgent, logger) {
    var abort, agent, buffer, bufferMsg, close, data, docState, failAuthentication, handleClose, handleMessage, handleOp, handleOpenCreateSnapshot, lastReceivedDoc, lastSentDoc, open, send;
	log = logger;
    data = {};
    agent = null;
    lastSentDoc = null;
    ...
    handleMessage = function(query) {
      ...
      if (error) {
        log.error ("Invalid query " + query + " from " + agent.sessionId + ": " + error);
        return abort();
      }
      if (query.doc === null) {
        ...
        if (!lastReceivedDoc) {
          log.error ("msg.doc missing in query " + query + " from " + agent.sessionId);
          return abort();
        }
      ...
      docState[_name = query.doc] || (docState[_name] = {
        queue: syncQueue(function(query, callback) {
          ...
          if (query.open === false) {
            ...
            log.warn(query, "Invalid query from " + agent.sessionId);
            abort();
            return callback();
          }
        })
    ...
    send = function(response, sessionId) {
      if (response.doc === lastSentDoc) {
        //delete response.doc;
      } else {
        lastSentDoc = response.doc;
      }
      return session.send(response, sessionId);
    };
    open = function(docName, version, callback) {
      ...
      docState[docName].listener = listener = function(opData) {
        ...
        return send(opMsg, agent.sessionId);
      };
    ...
    handleOpenCreateSnapshot = function(query, finished) {
      ...
      step1Create = function() {
	    session.add(docName, agent.sessionId);
        if (query.create !== true) {
	...
	log.error (error, "failAuthentication");
      session.send({
        auth: null,
        error: error
      });
    ...
    buffer = [];
	log.info('remove listener handleMessage and attaching to bufferMsg');
      ...
      if (typeof msg.auth !== 'undefined') {
        ...
        return createAgent(data, function(error, agent_) {
          ...
          if (error) {
            ...
            agent = agent_;
			log.info ("sending auth : " + agent.sessionId);
            session.send({
				doc: msg.docName,
