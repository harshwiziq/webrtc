...
RUN tar -xvf "node-v$NODE_VERSION-linux-x64.tar.gz" -C /usr/local --strip-components=1
RUN rm "node-v$NODE_VERSION-linux-x64.tar.gz" SHASUMS256.txt.asc SHASUMS256.txt

# install for paper.js
# RUN apt-get install libcairo2-dev libpango1.0-dev libjpeg62-dev libgif-dev
...
CMD		cd /session; node server.js session.info
